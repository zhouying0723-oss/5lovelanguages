...
function anonymousId(key: string) {
  try { const existing = localStorage.getItem(key); if (existing) return existing; const value = crypto.randomUUID(); localStorage.setItem(key, value); return value; }
  catch { return crypto.randomUUID(); }
}
function deviceType() { const width = window.innerWidth; return width < 600 ? "mobile" : width < 1024 ? "tablet" : "desktop"; }
function trackEvent(eventType: string, attemptId?: string, result?: unknown) {
  const body = JSON.stringify({ visitorId: anonymousId("love_language_visitor_id"), attemptId: attemptId || null, eventType, deviceType: deviceType(), result });
  fetch("/5lovelanguages/api/events", { method: "POST", headers: { "Content-Type": "application/json" }, body, keepalive: true }).catch(() => undefined);
}
...
function makeTieGroup(data: ReturnType<typeof calc>, mode: "receive" | "give"): TieGroup | null {
  const candidates = data.filter(item => data[0].score - item.score <= 2).map(item => item.key);
  return candidates.length < 2 ? null : { mode, candidates: candidates.slice(0, 5) };
}
...
export default function Home() {
  const [phase, setPhase] = useState<Phase>("intro"); const [index, setIndex] = useState(0); const [answers, setAnswers] = useState<number[]>(Array(30).fill(0)); const [tieGroupIndex, setTieGroupIndex] = useState(0); const [tieOpponentIndex, setTieOpponentIndex] = useState(1); const [tieChampion, setTieChampion] = useState<LoveKey | null>(null); const [tieChoices, setTieChoices] = useState<{ receive: LoveKey[]; give: LoveKey[] }>({ receive: [], give: [] }); const [tieWinners, setTieWinners] = useState<Partial<Record<"receive" | "give", LoveKey>>>({}); const [posterOpen, setPosterOpen] = useState(false); const canvasRef = useRef<HTMLCanvasElement>(null); const attemptIdRef = useRef(""); const completionTrackedRef = useRef(false);
  const rawReceive = useMemo(() => calc(answers, "receive"), [answers]); const rawGive = useMemo(() => calc(answers, "give"), [answers]);
  const tieGroups = useMemo(() => [makeTieGroup(rawReceive, "receive"), makeTieGroup(rawGive, "give")].filter((group): group is TieGroup => group !== null), [rawReceive, rawGive]);
  const receive = useMemo(() => rankWithTies(rawReceive, tieChoices.receive, tieWinners.receive), [rawReceive, tieChoices.receive, tieWinners.receive]); const give = useMemo(() => rankWithTies(rawGive, tieChoices.give, tieWinners.give), [rawGive, tieChoices.give, tieWinners.give]);
  const tieTotal = tieGroups.reduce((sum, group) => sum + group.candidates.length - 1, 0); const tieDone = tieGroups.slice(0, tieGroupIndex).reduce((sum, group) => sum + group.candidates.length - 1, 0) + tieOpponentIndex - 1;
  useEffect(() => { trackEvent("page_view"); }, []);
  useEffect(() => { if (phase !== "result" || completionTrackedRef.current) return; completionTrackedRef.current = true; trackEvent("quiz_complete", attemptIdRef.current, { receive: receive.map(item => ({ key: item.key, index: item.resultPct })), give: give.map(item => ({ key: item.key, index: item.resultPct })), usedTiebreak: tieGroups.length > 0 }); }, [phase, receive, give, tieGroups.length]);
  useEffect(() => { if (phase === "quiz" || phase === "tiebreak") window.scrollTo({ top: 0, behavior: "smooth" }); }, [index, tieGroupIndex, tieOpponentIndex, phase]);
  const choose = (n: number) => { const next = [...answers]; next[index] = n; setAnswers(next); window.setTimeout(() => { if (index < 29) setIndex(index + 1); else { const groups = [makeTieGroup(calc(next, "receive"), "receive"), makeTieGroup(calc(next, "give"), "give")].filter((group): group is TieGroup => group !== null); if (groups.length) { setTieChampion(groups[0].candidates[0]); setPhase("tiebreak"); } else setPhase("result"); } }, 180); };
  const chooseTie = (key: LoveKey) => { const group = tieGroups[tieGroupIndex]; setTieChoices(current => ({ ...current, [group.mode]: [...current[group.mode], key] })); window.setTimeout(() => { if (tieOpponentIndex < group.candidates.length - 1) { setTieChampion(key); setTieOpponentIndex(tieOpponentIndex + 1); return; } setTieWinners(current => ({ ...current, [group.mode]: key })); if (tieGroupIndex < tieGroups.length - 1) { const nextGroup = tieGroups[tieGroupIndex + 1]; setTieGroupIndex(tieGroupIndex + 1); setTieOpponentIndex(1); setTieChampion(nextGroup.candidates[0]); } else setPhase("result"); }, 180); };
  const startQuiz = () => { attemptIdRef.current = crypto.randomUUID(); completionTrackedRef.current = false; trackEvent("quiz_start", attemptIdRef.current); setPhase("quiz"); };
  const restart = () => { setAnswers(Array(30).fill(0)); setIndex(0); setTieGroupIndex(0); setTieOpponentIndex(1); setTieChampion(null); setTieChoices({ receive: [], give: [] }); setTieWinners({}); setPosterOpen(false); startQuiz(); };
  ...
  if (phase === "tiebreak") { const group = tieGroups[tieGroupIndex]; const challenger = group.candidates[tieOpponentIndex]; const champion = tieChampion ?? group.candidates[0]; return <main className="quiz-page tie-page"><header className="quiz-head"><div className="mini-brand"><span>五</span> 爱的语言</div><div className="step-text">再靠近一点 · {group.mode === "receive" ? "我如何接收爱" : "我如何给予爱"}</div><div className="count">{tieDone + 1} / {tieTotal}</div></header><div className="progress"><span style={{ width: `${((tieDone + 1) / tieTotal) * 100}%` }} /></div><section className="question-wrap tie-wrap"><div className="question-no">A GENTLE TIEBREAKER</div><h1>如果此刻只能保留一种，<br />哪一个对你更重要？</h1><p>这一题的选择会继续迎战下一个接近类型，直到产生明确的核心偏好。</p><div className="tie-options"><button onClick={() => chooseTie(champion)}><small>{LOVE[champion].name}</small><strong>{tieCopy[group.mode][champion]}</strong><span>选择这个 →</span></button><i>或</i><button onClick={() => chooseTie(challenger)}><small>{LOVE[challenger].name}</small><strong>{tieCopy[group.mode][challenger]}</strong><span>选择这个 →</span></button></div><div className="tie-note">胜出的选择会进入下一轮；最终胜者将成为结果页的“决胜后核心偏好”</div></section></main>; }
